...
class TestNotificationPreferences:
    ...
    def test_should_send_alert(self):
        ...
        assert (
            prefs.should_send_alert(AlertType.PRICE_DROP, alerts_sent_today=5) is True
        )

        # Should not send - disabled alert type
        assert (
            prefs.should_send_alert(AlertType.MARKET_UPDATE, alerts_sent_today=5)
            is False
        )

        # Should not send - at daily limit
        assert (
            prefs.should_send_alert(AlertType.PRICE_DROP, alerts_sent_today=10) is False
        )
