...
class MarketStatistics(BaseModel):
    ...
    # Location breakdown
    cities: Dict[str, int] = Field(
        default_factory=dict, description="Properties by city"
    )
    property_types: Dict[str, int] = Field(
        default_factory=dict, description="Properties by type"
    )

    # Price per sqm
    avg_price_per_sqm: Optional[float] = Field(
        None, description="Average price per square meter"
    )
...
class LocationInsights(BaseModel):
    ...
    price_comparison: Optional[str] = (
        None  # "above_average", "below_average", "average"
    )
...
class MarketInsights:
    ...
    def get_overall_statistics(self) -> MarketStatistics:
        ...
        # Average area
        avg_area = (
            self.df["area_sqm"].mean() if self.df["area_sqm"].notna().any() else None
        )
        ...
        return MarketStatistics(
            ...
            avg_area=(
                float(avg_area)
                if avg_area is not None and not np.isnan(avg_area)
                else None
            ),
    ...
    def get_location_insights(self, city: str) -> Optional[LocationInsights]:
        ...
        if len(city_df["rooms"]) > 0:
            most_common_rooms = (
                float(city_df["rooms"].mode().iloc[0])
                if len(city_df["rooms"].mode()) > 0
                else None
            )
    ...
    def get_monthly_price_index(
        ...
        if len(df) == 0:
            return pd.DataFrame(
                columns=["month", "avg_price", "median_price", "count", "yoy_pct"]
            )
        ...
        # Moving average
        grouped["avg_price_ma"] = (
            grouped["avg_price"].rolling(window=window, min_periods=1).mean()
        )
    ...
    def get_property_type_insights(
        self, property_type: str
    ) -> Optional[PropertyTypeInsights]:
        """
    ...
    def get_amenity_impact_on_price(self) -> Dict[str, float]:
        ...
        for amenity in amenities:
            amenity_mask = self.df[amenity].fillna(False).astype(bool)
            with_amenity = self.df[amenity_mask]["price"].mean()
            without_amenity = self.df[~amenity_mask]["price"].mean()

            if (
                pd.notna(with_amenity)
                and pd.notna(without_amenity)
                and without_amenity > 0
            ):
                percent_diff = float(
                    ((with_amenity - without_amenity) / without_amenity) * 100
                )
    ...
    def compare_locations(self, city1: str, city2: str) -> Dict[str, Any]:
        ...
        return {
            ...
            * 100,
            "cheaper_city": (
                city1 if insights1.avg_price < insights2.avg_price else city2
            ),
            "more_properties": (
    ...
    def get_cities_yoy(self, cities: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        if len(df) == 0:
            return pd.DataFrame(
                columns=["city", "month", "avg_price", "yoy_pct", "count"]
            )
        df["dt"] = pd.to_datetime(df["scraped_at"])
        df["month"] = df["dt"].dt.to_period("M").dt.to_timestamp()
        grouped = (
