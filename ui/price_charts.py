...
def create_price_by_location_chart(
    ...
    city_stats = (
        df.groupby("city")
        .agg({"price": ["mean", "median", "count", "min", "max"]})
        .reset_index()
    )
    ...
    if orientation == "h":
        ...
        fig.update_layout(xaxis_title="Median Price ($/month)", yaxis_title="City")
    else:
        fig = go.Figure(
            go.Bar(
                x=city_stats["city"],
        ...
        fig.update_layout(xaxis_title="City", yaxis_title="Median Price ($/month)")
...
def create_price_amenity_scatter(
    properties: PropertyCollection, title: str = "Price vs Amenities"
) -> go.Figure:
    """
...
def create_price_per_sqm_chart(
    properties: PropertyCollection, title: str = "Price per Square Meter by Location"
) -> go.Figure:
    """
...
def create_price_trend_line_chart(
    ...
    df_sorted["rolling_avg"] = (
        df_sorted["price"].rolling(window=window, center=True).mean()
    )
...
def create_price_comparison_chart(
    properties: List[Property], title: str = "Property Price Comparison"
) -> go.Figure:
    """
